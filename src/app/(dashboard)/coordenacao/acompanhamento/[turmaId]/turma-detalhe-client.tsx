...
import { SITUACAO_CFG, ETAPA_CFG, type Turma } from "@/lib/mock-data/professor";
import type { TotaisMap } from "@/lib/queries/notas";
import { GradeNotas, type FrequenciasGrade } from "@/components/professor/GradeNotas";

// UI do detalhe da turma no Acompanhamento (coordenação). Turma REAL do banco (via props);
// notas em modo somente leitura. A frequência exibida na grade é a calculada.
export default function AcompanhamentoTurmaClient({
  turma,
  totais,
  frequencias,
}: {
    ...
    <div className="mx-auto max-w-4xl space-y-5">
      ...
      <div>
        ...
        <p className="mt-0.5 text-sm text-[#4B5563]">
          {[turma.polo, turma.professorResponsavel, turma.periodoLetivo].filter(Boolean).join(" · ")}
        </p>
      </div>
      ...
      {aba === "notas" && (
        <GradeNotas turma={turma} readOnly totais={totais} frequencias={frequencias} />
      )}
