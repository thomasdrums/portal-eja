...
import { SITUACAO_CFG, type SituacaoAluno } from "@/lib/mock-data/professor";
import type { TurmaResumoRow } from "@/lib/queries/relatorios";
...
export default function RelatorioPorTurmaClient({ turmas }: { turmas: TurmaResumoRow[] }) {
  const [polo, setPolo] = useState("");
  const [prof, setProf] = useState("");
  const [turmaId, setTurmaId] = useState("");

  const polos = useMemo(() => [...new Set(turmas.map((t) => t.poloNome))].sort(), [turmas]);

  const professores = useMemo(
    () => [
      ...new Set(
        turmas
          .filter((t) => !polo || t.poloNome === polo)
  ...
  const turmasOpcoes = useMemo(
    () =>
      turmas
        .filter(
          (t) =>
            (!polo || t.poloNome === polo) &&
            (!prof || t.professores.includes(prof)),
        )
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <div className="grid grid-cols-1 gap-3 rounded-lg border border-[#E5E7EB] bg-white p-4 shadow-[0_1px_3px_rgba(0,0,0,0.06)] sm:grid-cols-3">
        <Select
          label="Polo"
          value={polo}
          onChange={(v) => { setPolo(v); setProf(""); setTurmaId(""); }}
          options={polos.map((p) => ({ value: p, label: p }))}
        />
        <Select
          label="Professor"
          value={prof}
          onChange={(v) => { setProf(v); setTurmaId(""); }}
          options={professores.map((p) => ({ value: p, label: p }))}
        />
        <Select label="Turma" value={turmaId} onChange={setTurmaId} options={turmasOpcoes} />
      </div>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-lg bg-[#009640] p-6 text-white">
              ...
              <p className="text-sm text-white/80">
                {[turma.poloNome, turma.professores.join(", "), turma.periodoLetivo].filter(Boolean).join(" · ")}
              </p>
            </div>
