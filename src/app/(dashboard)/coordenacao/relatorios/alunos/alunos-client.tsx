...
import type { RelatorioAlunosData } from "@/lib/queries/relatorios";

// Situações exibidas no filtro (mesma lista de hoje).
...
// Config de cor/rótulo da situação, com fallback para valores fora da lista (ex.: INATIVO).
...
export default function RelatorioAlunosClient({ dados }: { dados: RelatorioAlunosData }) {
  ...
  const filtered = useMemo(() =>
    ...
    ), [dados.linhas, polo, turma, cidade, situacao]);
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 rounded-lg border border-[#E5E7EB] bg-white p-4 shadow-[0_1px_3px_rgba(0,0,0,0.06)] sm:grid-cols-4">
        ...
        <Select label="Turma"    value={turma}    onChange={setTurma}    options={dados.turmas}  />
        <Select label="Cidade"   value={cidade}   onChange={setCidade}   options={dados.cidades} />
        <Select
          label="Situação"
          value={situacao}
          onChange={setSituacao}
          options={SITUACOES}
          renderOption={(v) => cfgDe(v as SituacaoAluno).label}
        />
      </div>
