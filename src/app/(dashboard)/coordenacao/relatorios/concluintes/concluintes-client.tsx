...
import type { RelatorioConcluintesData } from "@/lib/queries/relatorios-avaliacao";
...
export default function ConcluintesClient({ dados }: { dados: RelatorioConcluintesData }) {
  const [polo, setPolo] = useState("");
  const [mes,  setMes]  = useState("");
  const [ano,  setAno]  = useState("");

  const filtered = useMemo(() =>
    dados.linhas.filter((a) =>
      (!polo || a.poloNome === polo) &&
      (!mes  || a.dataConclusao === mes) &&
      (!ano  || a.dataConclusao?.endsWith(ano))
    ), [dados.linhas, polo, mes, ano]);

  const porTurma = dados.porTurma.filter((t) => t.concluintes > 0);
    ...
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        <div className="rounded-lg border border-[#E5E7EB] bg-white p-5 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
          ...
          <p className="mt-1 text-3xl font-extrabold text-gray-900">{dados.totalConcluintes}</p>
        </div>
        <div className="rounded-lg border border-[#E5E7EB] bg-white p-5 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
          <p className="text-xs font-semibold text-[#4B5563]">Taxa de Conclusão</p>
          <p className="mt-1 text-3xl font-extrabold text-[#009640]">{dados.pctConclusao}%</p>
        </div>
        <div className="col-span-2 rounded-lg border border-[#E5E7EB] bg-white p-5 shadow-[0_1px_3px_rgba(0,0,0,0.06)] sm:col-span-1">
          ...
          <div className="space-y-2">
            {porTurma.length === 0 ? (
              <p className="text-xs text-gray-400">Sem concluintes ainda.</p>
            ) : porTurma.map((t) => {
              const pct = t.total > 0 ? Math.round((t.concluintes / t.total) * 100) : 0;
              return (
                <div key={t.turma}>
      ...
      <div className="grid grid-cols-3 gap-3 rounded-lg border border-[#E5E7EB] bg-white p-4 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
        <Select label="Polo" value={polo} onChange={setPolo} options={dados.polos} />
        <Select label="Mês"  value={mes}  onChange={setMes}  options={dados.meses} />
        <Select label="Ano"  value={ano}  onChange={setAno}  options={dados.anos}  />
      </div>
